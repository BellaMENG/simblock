...
import static simblock.settings.SimulationConfiguration.NUM_OF_COMMITTEES;
...
public class BitcoinCoreTable extends AbstractRoutingTable {
    ...
    /**
     * overwrite the initTable function. This time, form a routing table within the committee
     */
    public void initTable(int committeeID) {
        ArrayList<Integer> candidates = new ArrayList<>();
        int shardSize = NUM_OF_NODES/NUM_OF_COMMITTEES;
        int startID = committeeID*shardSize;
        for (int i = startID; i < Math.min(startID+shardSize, NUM_OF_NODES); ++i) {
            candidates.add(i);
        }
        Collections.shuffle((candidates));
        for (int candidate : candidates) {
