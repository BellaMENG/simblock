...
public class ProofOfWork extends AbstractConsensusAlgo {
	 ...
	 */
	//public static int mintingCount = 0;
	...
	public MiningTask minting() {
		...
		//System.out.printf("%d: p is %f, u is %f, interval is %d%n", ProofOfWork.mintingCount, p, u, interval);
		//ProofOfWork.mintingCount += 1;
		return p <= Math.pow(2, -53) ? null
				: new MiningTask(selfNode, (long) (Math.log(u) / Math.log(1.0 - p) / selfNode.getMiningPower()),
						difficulty, numberOfTx);
	}

	/**
	 * Tests if the receivedBlock is valid with regards to the current block. The
	 * receivedBlock is valid if it is an instance of a Proof of Work block and the
	 * received block needs to have a bigger difficulty than its parent next
	 * difficulty and a bigger total difficulty compared to the current block.
	 *
